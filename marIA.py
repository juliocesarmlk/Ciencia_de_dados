...
def selecao(individuos, n = 3, p = 0.75):
    escolhidos = []
    for _ in range(len(individuos)):
        torneio = random.sample(individuos, n)
        torneio.sort(key=lambda individuo: individuo.fitness, reverse=True)

        for i in range(n):
            if random.random() < p:
                escolhidos.append(torneio[i])
                break
            p *= (1 - p)
        return escolhidos
    
def cruzamento(pai1, pai2):
    ponto = random.randint(1, len(pai1.acoes) -1)
    filho1_acoes = pai1.acoes[:ponto] + pai2.acoes[ponto:]
    filho2_acoes = pai1.acoes[ponto:] + pai2.acoes[:ponto]
    filho1 = Individuo()
    filho2 = Individuo()
    filho1.acoes = filho1_acoes
    filho2.acoes = filho2_acoes
    return filho2, filho1

def mutacao(individuo, taxa_mutacao=0.1):
    for i in range(len(individuo.acoes)):
        if random.random() < taxa_mutacao:
            novo_acao = (random.randint(0, 2), random.randint(1, 10))
            individuo.acoes[i] = novo_acao
